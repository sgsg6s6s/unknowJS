...
export const config: { [key: string]: string[][] } = {
  原型: [
    ...
    [
      'p',
      '2. new func()或者func()会执行函数func的大括号内的代码，不过两者还是有区别的'
    ],
    ...
    [
      'p',
      '2. 函数对象包含__proto__和函数体中`this.XX`增加的属性，后续依然可以为函数对象增加属性'
    ],
    [
      'p',
    ...
    [
      'p',
      '`自有属性`是指不包括在原型链中，但可以被getOwnPropertyNames获取的属性'
    ],
    ...
    [
      'h2',
      "设置属性，例如:obj.a = 'b', 需要先查询属性，再决定设置值到哪个位置"
    ],
    ...
    [
      'p',
      ' 4. 如果自有属性不存在a，在原型链中找到，并且该属性不是描述符，修改原型链的属性a的值'
    ]
  ]
}
