...
export const config: { [key: string]: string[][] } = {
  词法作用域: [
    ...
    ['p', '一般情况，词法分析器处理后，保持书写代码的作用域结构，但欺骗词法作用域的代码写法例外'],
    ['h1', '查找'],
    ['p', '作用域查找到第一个匹配的标识符就会停止，应对同名的标识符产生遮蔽效应'],
    ['p', 'window.a可以访问全局作用域上被遮蔽的a标识符，但中间的作用域的a是访问不到的'],
    ['h1', '欺骗词法（禁用，性能变差,引擎无法在编译时对作用域查找进行优化）'],
    ['h2', '1. eval'],
    ['p', '还有一些效果类似eval，可以欺骗词法分析，setTimeout、setInterval、new Function()'],
    ['h2', ' 2. with'],
    ['p', ' with包括的对象属于with的作用域，with无关的声明会加到with所属的作用域下'],
    ['h1', '动态作用域'],
    [
      'p',
